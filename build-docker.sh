...
sudo docker container run --name meugeninua.postgres -e POSTGRES_PASSWORD=horoscopes -e POSTGRES_USER=horoscopes -e POSTGRES_DB=horoscopes --mount "type=bind,src=/var/postgres/data,dst=/var/lib/postgresql/data" --network horoscopes-network -d postgres:9.6
