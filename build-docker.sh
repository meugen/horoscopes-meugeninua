...
sudo docker container run --name meugeninua.postgres -e POSTGRES_PASSWORD=horoscopes -e POSTGRRESS_USER=horoscopes -e POSTGRESS_DB=horoscopes --mount "type=bind,src=/var/postgres/data,dst=/var/lib/postgresql/data" --network horoscopes-network -d postgres:9.6
