...
PGADMIN_SERVERS_JSON=/var/postgres/servers.json
...
[ $? -eq 0 ] || exit 1
sudo docker login
sudo docker rm -f meugeninua.horoscopes meugeninua.postgres meugeninua.pgadmin
...
sudo docker container run --name meugeninua.pgadmin -e PGADMIN_DEFAULT_EMAIL=pgadmin@example.com -e PGADMIN_DEFAULT_PASSWORD=welc0me -e PGADMIN_LISTEN_PORT=5480 -v $PGADMIN_SERVERS_JSON:/pgadmin4/servers.json --network horoscopes-network -d -p 5480:5480 dpage/pgadmin4:latest